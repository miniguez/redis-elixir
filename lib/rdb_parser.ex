defmodule Server.RDBParser do
  ...
  defp parse_opcodes(<<>>, acc) do # Should ideally be caught by EOF
    Logger.warn("Reached end of binary unexpectedly without EOF marker.")
  ...
  defp parse_key_value_pair(data, value_type, expiry_ms, acc) do
    current_time_ms = System.monotonic_time(:millisecond) # Using monotonic for internal expiry check during parsing
  ...
  def parse_string_encoded(binary_data) do
    case parse_length_encoded_int(binary_data) do
      ...
      {:ok, _length, _rest, {:compressed_string}} ->
        ...
        Logger.warn("LZF compressed string encountered, not supported for keys yet.")
      ...
      _ ->
        {:error, "Unknown string encoding format"}
    end
  end
end
